...
class Player ##purpose to interact with player/computer
  ...
  def generate_order
    ...
    @order = ["1", "4", "2", "4", "5"]
    # Array.new(5) { color_array.sample }
  end
  ...
  def initialize
    ...
    def correct_order? 
      @player.order.each_with_index do
        |v, i|
        if v == @rows[@player.round].guesses[i]
          @rows[@player.round].correct.push(v)
        end
      end
    end

    def correct_color?
      rows[@player.round].guesses.each_with_index do
        |v, i|
        # binding.pry
        @player.order.select do |v|
          if (v == @rows[@player.round].guesses[i]) && (@rows[@player.round].correctcolor.count(v) < @player.order.count(v))
            @rows[@player.round].correctcolor.push(v)
          end
        end
      end
    end  
    ...
    def wrong?
      @player.order.each_with_index do
        |v, i|
          @rows[@player.round].wrong.push(v)
       end
    end
